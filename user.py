...
# Import from utils
import utils # Import utils to call its functions
from utils import (
...
# Emojis (Keep as is)
EMOJI_CITY = "🏙️"; EMOJI_DISTRICT = "🏘️"; EMOJI_PRODUCT = "💎"; EMOJI_HERB = "🌿"
EMOJI_PRICE = "💰"; EMOJI_QUANTITY = "🔢"; EMOJI_BASKET = "🛒"; EMOJI_PROFILE = "👤"
EMOJI_REFILL = "💸"; EMOJI_REVIEW = "📝"; EMOJI_PRICELIST = "📋"; EMOJI_LANG = "🌐"
EMOJI_BACK = "⬅️"; EMOJI_HOME = "🏠"; EMOJI_SHOP = "🛍️"; EMOJI_DISCOUNT = "🏷️"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Synchronously clear basket based on DB state *after* fetching user data
    ...
    welcome_part = welcome_template.format(username=helpers.escape_markdown(username, version=2) if username else f"User_{user_id}") # Escape username just in case
    full_welcome = (
        f"{welcome_part}\n\n{status_line}\n{balance_line}\n"
        f"{purchases_line}\n{basket_line}\n\n{shopping_prompt}\n\n⚠️ {refund_note}"
    )
    ...
    if is_callback:
        ...
        try:
             current_message_text = query.message.text if query.message else None
             current_markup = query.message.reply_markup if query.message else None
             if current_message_text != full_welcome or current_markup != reply_markup:
                  await query.edit_message_text(full_welcome, reply_markup=reply_markup, parse_mode=None)
             else:
                  await query.answer() # Acknowledge callback if message is identical
              ...
              if "message is not modified" not in str(e).lower():
                  ...
                  await send_message_with_retry(context.bot, chat_id, full_welcome, reply_markup=reply_markup, parse_mode=None)
                   ...
                   await query.answer() # Acknowledge if it was a "not modified" error
        except Exception as e:
             logger.error(f"Unexpected error editing start message: {e}", exc_info=True)
             # Attempt to send a new message as fallback
             await send_message_with_retry(context.bot, chat_id, full_welcome, reply_markup=reply_markup, parse_mode=None)
    else:
        await send_message_with_retry(context.bot, chat_id, full_welcome, reply_markup=reply_markup, parse_mode=None)
...
async def handle_shop(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    # Use the globally loaded CITIES dictionary
    if not CITIES:
    ...
    try:
        ...
        for c_id in sorted_city_ids:
             ...
             if city_name:
                 keyboard.append([InlineKeyboardButton(f"{EMOJI_CITY} {city_name}", callback_data=f"city|{c_id}")])
             else:
        ...
        logger.info(f"handle_shop: Successfully displayed city selection for user {user_id}.")
    except telegram_error.BadRequest as e:
         if "message is not modified" not in str(e).lower():
        ...
        try:
            keyboard = [[InlineKeyboardButton(f"{EMOJI_HOME} {home_button_text}", callback_data="back_start")]]
            await query.edit_message_text("❌ An error occurred while displaying cities.", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=None)
...
async def handle_city_selection(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    if not params or len(params) < 1:
        ...
        return
    city_id = params[0]
    city_name = CITIES.get(city_id) # Use globally loaded CITIES
    if not city_name:
        error_city_not_found = lang_data.get("error_city_not_found", "Error: City not found.")
        await query.edit_message_text(f"❌ {error_city_not_found} Please select again.", parse_mode=None)
    ...
    districts_in_city = DISTRICTS.get(city_id, {}) # Use globally loaded DISTRICTS
...
async def handle_district_selection(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    if not city_name or not district_name:
        ...
        await query.edit_message_text(f"❌ {error_district_city_not_found} Please select again.", parse_mode=None)
        logger.warning(f"City {city_id} or District {dist_id} not found in loaded data.")
        # Go back to city selection as district depends on city
    ...
    try:
        conn = get_db_connection()
        c = conn.cursor()
        # Fetch distinct product types available in this specific city/district
        c.execute("""
        ...
        logger.error(f"DB error fetching product types for {city_name}/{district_name}: {e}", exc_info=True)
...
async def handle_type_selection(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    if not city_name or not district_name:
        error_district_city_not_found = lang_data.get("error_district_city_not_found", "Error: District or city not found.")
        await query.edit_message_text(f"❌ {error_district_city_not_found} Please select again.", parse_mode=None)
        # Go back to city selection if location is invalid
        return await handle_shop(update, context)
    ...
    try:
        conn = get_db_connection()
        c = conn.cursor()
        # Group by size and price to show distinct options, count available instances
        c.execute("""
            SELECT size, price, COUNT(*) as count_available
            FROM products
            WHERE city = ? AND district = ? AND product_type = ? AND available > reserved
            GROUP BY size, price
            ORDER BY price, size -- Sort primarily by price, then size
        """, (city_name, district_name, p_type))
        products = c.fetchall() # List of Row objects
        ...
        if not products:
            ...
            for row in products:
                ...
                # Ensure price for callback data uses standard float representation
                price_str_callback = str(float(price)) # Convert to float then back to string for consistency
                button_text = f"{product_emoji} {size} ({price_str_formatted}€) - {available_label_short}: {count}"
                # Pass type, size, and price in callback data
        ...
        logger.error(f"DB error fetching products for {city_name}/{district_name}/{p_type}: {e}", exc_info=True)
...
async def handle_product_selection(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    try:
        # Convert price string back to float for DB query
    ...
    if not city_name or not district_name:
        ...
        await query.edit_message_text(f"❌ {error_location_mismatch} Please start over.", parse_mode=None)
    ...
    try:
        conn = get_db_connection()
        c = conn.cursor()
        # Check current availability for this specific variant
        c.execute("""
            SELECT COUNT(*) as count FROM products
            WHERE city = ? AND district = ? AND product_type = ? AND size = ? AND price = ? AND available > reserved
        """, (city_name, district_name, p_type, size, price))
        ...
        if available_count <= 0:
            ...
            msg = (
               f"{EMOJI_CITY} {city_name} | {EMOJI_DISTRICT} {district_name}\n"
            ...
            # Ensure price string in callback is consistent
        ...
        logger.error(f"DB error checking availability for {city_name}/{district_name}/{p_type}/{size}: {e}", exc_info=True)
...
async def handle_add_to_basket(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    try:
        price = float(price_str) # Convert price back to float
    except ValueError:
    ...
    try:
        ...
        c = conn.cursor()
        # --- Transaction Start ---
        c.execute("BEGIN EXCLUSIVE") # Lock for atomic check and update

        # Find ONE available product instance matching criteria
        c.execute("""
            SELECT id FROM products
            WHERE city = ? AND district = ? AND product_type = ? AND size = ? AND price = ? AND available > reserved
            ORDER BY id -- Or some other logic if needed, e.g., RANDOM()
            LIMIT 1
        """, (city_name, district_name, p_type, size, price))
        product_row = c.fetchone() # Gets one Row object or None

        if not product_row:
            # Item became unavailable between viewing and clicking add
        ...
        product_id_reserved = product_row['id'] # Get the specific ID

        # Increment reservation count for the specific product ID found
        reserve_update_result = c.execute("UPDATE products SET reserved = reserved + 1 WHERE id = ?", (product_id_reserved,))
        if reserve_update_result.rowcount == 0:
            # Should not happen if SELECT worked, but safety check
            conn.rollback()
            logger.error(f"Failed to increment reservation for product {product_id_reserved} (user {user_id}) despite finding it.")
            await query.edit_message_text(f"❌ {error_adding_unexpected}", parse_mode=None)
            return

        # Fetch current basket string to append
        ...
        timestamp = time.time()
        # Store specific product ID and timestamp
        ...
        # Add item with price to context for total calculation
        ...
        original_total = sum(Decimal(str(item['price'])) for item in current_basket_list) # Use Decimal
        final_total = original_total # Use Decimal
        discount_amount = Decimal('0.0') # Use Decimal
        applied_discount_info = context.user_data.get('applied_discount')
        pay_msg_str = ""

        if applied_discount_info:
             # Use the moved function from utils
             code_valid, _, discount_details = utils.validate_discount_code(applied_discount_info['code'], float(original_total))
        ...
        if discount_amount > Decimal('0.0'):
             ...
             # Note: Markdown might not render in all clients, keep it simple
             pay_msg_str = f"Original: {original_total_str} EUR\nDiscount: -{discount_amount_str} EUR\n{pay_msg_str}"
        ...
        expiry_time_str = expiry_dt.strftime('%H:%M:%S') # Example format
        ...
        district_btn_text = district_name[:15] # Shorten district name for button if needed
        keyboard = [
...
async def handle_profile(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    try:
        ...
        if not result:
            ...
            await query.edit_message_text("❌ Error: Could not load your profile. Try /start again.", parse_mode=None)
            return
        balance, purchases = result['balance'], result['total_purchases']

        clear_expired_basket(context, user_id) # Sync function
        ...
        await query.edit_message_text("❌ Error: Failed to Load Profile Data", parse_mode=None)
    except telegram_error.BadRequest as e:
        if "message is not modified" not in str(e).lower():
             logger.error(f"Unexpected BadRequest error in handle_profile for user {user_id}: {e}", exc_info=True)
             await query.edit_message_text("❌ Error: An unexpected issue occurred.", parse_mode=None)
        else:
             await query.answer()
    except Exception as e:
...
# --- Discount Validation (Function moved to utils.py) ---
# REMOVED validate_discount_code function definition from here
...
async def handle_view_basket(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    if not basket:
        ...
        keyboard = [[InlineKeyboardButton(f"{EMOJI_SHOP} {shop_button_text}", callback_data="shop"),
                     InlineKeyboardButton(f"{EMOJI_HOME} {home_button_text}", callback_data="back_start")]]
        try:
            await query.edit_message_text(full_empty_msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=None)
        except telegram_error.BadRequest as e:
    ...
    try:
        ...
        if discount_code_to_revalidate:
            ...
            code_valid, validation_message, discount_details = utils.validate_discount_code(discount_code_to_revalidate, float(original_total))
            if code_valid and discount_details:
                discount_amount = Decimal(str(discount_details['discount_amount']))
                final_total = Decimal(str(discount_details['final_total']))
        ...
        await query.edit_message_text("❌ Error: Failed to Load Basket Details", parse_mode=None)
    except Exception as e:
...
async def handle_user_discount_code_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message or not update.message.text:
        ...
        await send_message_with_retry(context.bot, chat_id, send_text_please, parse_mode=None)
        return
    ...
    if not entered_code:
        ...
        await send_message_with_retry(context.bot, chat_id, no_code_entered_msg, parse_mode=None)
        keyboard = [[InlineKeyboardButton(view_basket_button_text, callback_data="view_basket")]]
        await send_message_with_retry(context.bot, chat_id, returning_to_basket_msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=None)
        return

    # --- Re-calculate current basket total ---
    clear_expired_basket(context, user_id)
    basket = context.user_data.get("basket", [])
    original_total = Decimal('0.0')
    conn = None
    if basket:
         try:
            ...
            original_total = sum(prices_dict.get(item['product_id'], Decimal('0.0')) for item in basket if item['product_id'] in prices_dict)
         except sqlite3.Error as e:
              ...
              await send_message_with_retry(context.bot, chat_id, f"❌ {error_calc_total}", parse_mode=None)
              kb = [[InlineKeyboardButton(view_basket_button_text, callback_data="view_basket")]]
              await send_message_with_retry(context.bot, chat_id, returning_to_basket_msg, reply_markup=InlineKeyboardMarkup(kb), parse_mode=None)
              return
         finally:
              if conn: conn.close()
    else:
        basket_empty_no_discount = lang_data.get("basket_empty_no_discount", "Your basket is empty. Cannot apply discount code.")
        await send_message_with_retry(context.bot, chat_id, basket_empty_no_discount, parse_mode=None)
        kb = [[InlineKeyboardButton(view_basket_button_text, callback_data="view_basket")]]
        await send_message_with_retry(context.bot, chat_id, returning_to_basket_msg, reply_markup=InlineKeyboardMarkup(kb), parse_mode=None)
        return
    ...
    # --- Validate and Apply ---
    # Use the function from utils
    code_valid, message, discount_details = utils.validate_discount_code(entered_code, float(original_total))
...
async def handle_remove_from_basket(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    if found_item_index != -1:
        ...
        new_basket_context = list(current_basket_context)
    ...
    try:
        ...
        if not context.user_data['basket']:
            context.user_data.pop('applied_discount', None)
             ...
             basket_total_after_removal = sum(Decimal(str(item['price'])) for item in context.user_data['basket'])
             # Use the function from utils
             code_valid, _, _ = utils.validate_discount_code(applied_discount_info['code'], float(basket_total_after_removal))
             if not code_valid:
                  context.user_data.pop('applied_discount', None)
        ...
        await query.edit_message_text("❌ Error: Failed to remove item due to database issue.", parse_mode=None)
        return
    except Exception as e:
        if conn and conn.in_transaction: conn.rollback()
        logger.error(f"Unexpected error removing item {product_id_to_remove} for user {user_id}: {e}", exc_info=True)
        await query.edit_message_text("❌ Error: An unexpected issue occurred while removing the item.", parse_mode=None)
        return
    finally:
        if conn: conn.close()
...
async def handle_clear_basket(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    try:
        ...
        c = conn.cursor()
        c.execute("BEGIN")
        # Clear basket string in DB first
        ...
        await query.edit_message_text("❌ Error: Failed to clear basket due to database issue.", parse_mode=None)
    except Exception as e:
        if conn and conn.in_transaction: conn.rollback()
...
async def handle_view_history(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    if not history:
        ...
        for i, purchase in enumerate(history):
            try:
                # Use timezone-aware parsing if possible, fallback otherwise
                date_obj = datetime.fromisoformat(purchase['purchase_date'])
                date_str = date_obj.strftime('%Y-%m-%d %H:%M') # Local time format
    ...
    if params: # Language code passed in callback data
        ...
        if new_lang in LANGUAGES:
            try:
                ...
                # Refresh start menu with new language
                await start(update, context)
                language_set_answer = LANGUAGES.get(new_lang, {}).get("language_set_answer", "Lang set!")
            ...
            invalid_lang_answer = lang_data.get("invalid_language_answer", "Invalid lang.")
...
async def handle_price_list(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    if not CITIES:
        ...
        await query.edit_message_text(f"{EMOJI_CITY} {no_cities_msg}", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=None)
        return
...
async def handle_price_list_city(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    try:
        ...
        c = conn.cursor()
        c.execute("""
            SELECT product_type, size, price, district, COUNT(*) as quantity
            FROM products WHERE city = ? AND available > reserved
        ...
        await query.edit_message_text(f"❌ {error_loading_prices_db}", parse_mode=None)
    except Exception as e:
...
async def handle_reviews_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    await query.edit_message_text(review_prompt, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=None)
...
async def handle_refill(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    MIN_REFILL_AMOUNT = 1.0 # Set minimum amount
    ...
    try:
        ...
        if "message is not modified" not in str(e).lower():
            ...
            await query.answer() # Still answer callback if send worked
        else:
            await query.answer(enter_amount_answer)
    except Exception as e:
...
async def handle_refill_amount_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    MIN_REFILL_AMOUNT = 1.0 # Consistent minimum amount
    ...
    try:
        ...
        if refill_amount < MIN_REFILL_AMOUNT:
            ...
            return # Keep state as is for user to retry
        if refill_amount > 10000: # Reasonable upper limit
        ...
        # Define supported assets for NOWPayments deposits
        supported_assets = ['btc', 'eth', 'ltc', 'sol', 'usdt', 'usdc', 'ton'] # Lowercase for NOWPayments API
        asset_buttons = []
        row = []
        for asset in supported_assets:
            # Use uppercase for button display
            row.append(InlineKeyboardButton(asset.upper(), callback_data=f"select_refill_crypto|{asset}"))
        ...
        # Add a title to the crypto selection message
        select_deposit_title = lang_data.get("select_deposit_crypto", "💳 Choose Deposit Method")
        final_message = f"{select_deposit_title}\n\n{choose_crypto_msg}"

        await send_message_with_retry(
            context.bot, chat_id, final_message,
