...
from utils import (
    ...
    SECONDARY_ADMIN_IDS, log_admin_action, # Added log_admin_action
            ...
            if download_errors > 0:
             ...
             if temp_dir and await asyncio.to_thread(os.path.exists, temp_dir):
                 try:
                     ...
                     logger.error(f"Error removing temp_dir {temp_dir} after download loop error: {rm_e}")
                 temp_dir = None
...
async def _process_collected_media(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Use application.user_data which is globally accessible within the application
    ...
    # Clean up user_data associated with this group ID
    user_data.pop('collecting_media_group_id', None)
    ...
    # Call the confirmation preparation function
...
async def handle_adm_drop_details_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Use application.user_data directly
    ...
    if media_group_id:
        ...
        if media_group_id not in user_specific_data['collected_media']:
            ...
            # Don't set collecting_media_group_id here, let the job handle cleanup
        ...
        # Store caption if provided, even if it overwrites a previous one for the same group
        if text:
        ...
        # Always reschedule the job to handle potential further messages in the group
        ...
        if hasattr(context, 'job_queue') and context.job_queue:
            context.job_queue.run_once(
                ...
                name=job_name,
                job_kwargs={'misfire_grace_time': 15} # Allow some delay if bot was busy
            )
        ...
        # Received a single message (text or single media with caption)
        # Cancel any pending media group job for this user (edge case)
        pending_group_id = user_specific_data.pop('collecting_media_group_id', None)
        if pending_group_id:
            pending_job_name = f"process_media_group_{user_id}_{pending_group_id}"
            remove_job_if_exists(pending_job_name, context)
            # Also clear any collected media data for that potential group
            if 'collected_media' in user_specific_data:
                user_specific_data['collected_media'].pop(pending_group_id, None)
                if not user_specific_data['collected_media']:
                    user_specific_data.pop('collected_media', None)
            logger.warning(f"Received single message from user {user_id} while potentially collecting media group {pending_group_id}. Processing single message instead.")
...
async def handle_admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🤝 Manage Resellers", callback_data="manage_resellers_menu|0")], # <-- ADDED
        [InlineKeyboardButton("🏷️ Manage Reseller Discounts", callback_data="manage_reseller_discounts_select_reseller|0")], # <-- ADDED
        [InlineKeyboardButton("🏷️ Manage General Discounts", callback_data="adm_manage_discounts")], # <-- RENAMED
    ...
    if query:
        try:
            ...
            if "message is not modified" not in str(e).lower():
                ...
                # Try sending instead of editing if edit fails badly
...
async def handle_adm_city(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    load_all_data() # Ensure cities are fresh
    if not CITIES:
...
async def handle_adm_dist(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    load_all_data() # Ensure districts are fresh
...
async def handle_adm_type(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    if not PRODUCT_TYPES:
        return await query.edit_message_text("No product types configured. Add types via 'Manage Product Types'.", parse_mode=None,
                                             reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("➕ Add New Type", callback_data="adm_add_type")],
                                                                                 [InlineKeyboardButton("⬅️ Back to Districts", callback_data=f"adm_dist|{city_id}")]]))
...
async def handle_confirm_add_drop(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    # Use application.user_data which is globally accessible
    user_specific_data = context.application.user_data.get(user_id, {})
    ...
    if not all([city, district, p_type, size, price is not None]):
        ...
        if temp_dir and await asyncio.to_thread(os.path.exists, temp_dir):
            try: await asyncio.to_thread(shutil.rmtree, temp_dir, ignore_errors=True)
            except Exception as rm_e: logger.error(f"Error removing temp_dir {temp_dir} during data validation fail: {rm_e}")
    ...
    try:
        ...
        c.execute(
            """INSERT INTO products
                ...
                VALUES (?, ?, ?, ?, ?, ?, 1, 0, ?, ?, ?)""",
            (city, district, p_type, size, price, original_text, ADMIN_ID, datetime.now().isoformat())
        )
        ...
        if temp_dir and await asyncio.to_thread(os.path.exists, temp_dir):
            try:
                ...
                logger.error(f"Error removing temp_dir {temp_dir} after successful add: {rm_e}")


        await query.edit_message_text("✅ Drop Added Successfully!", parse_mode=None)

        # Try to get context for 'Add Another' button
        ...
        if temp_dir and await asyncio.to_thread(os.path.exists, temp_dir):
            try:
                ...
                logger.info(f"Cleaned up temporary directory after error: {temp_dir}")
                 ...
                 logger.error(f"Error removing temp_dir {temp_dir} after drop add error: {rm_e}")

    finally:
        if conn: conn.close()
        # Always clean up user context for the drop process
...
async def cancel_add(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    # Clear relevant keys from user_data
    keys_to_clear = [
    ...
    # Cancel any pending media group processing job
    if 'collecting_media_group_id' in user_specific_data: # Check if key exists before popping
    ...
    elif update.message: # If cancellation was triggered by a message somehow
         await send_message_with_retry(context.bot, update.message.chat_id, "Add product cancelled.", parse_mode=None)
    else: # Internal cancellation or unknown trigger
...
async def handle_adm_manage_districts_city(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    # Reload districts directly from DB for this view
    districts_in_city = {}
    conn = None
    try:
        conn = get_db_connection() # Use helper
        ...
        districts_in_city = {} # Fallback to empty if DB error
    finally:
        if conn: conn.close()
    ...
    try:
        ...
        if not product_types_in_dist:
             ...
             return await query.edit_message_text(f"No product types found in {city_name} / {district_name}.",
                                     reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=None)
        keyboard = []
        load_all_data() # Ensure emojis are fresh
...
async def handle_adm_delete_type(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    try:
        ...
        # Check reseller discount usage
        c.execute("SELECT COUNT(*) FROM reseller_discounts WHERE product_type = ?", (type_name,))
        reseller_discount_count = c.fetchone()[0]

        usage_msgs = []
        if product_count > 0: usage_msgs.append(f"{product_count} product(s)")
        if reseller_discount_count > 0: usage_msgs.append(f"{reseller_discount_count} reseller discount rule(s)")

        if usage_msgs:
            usage_str = " and ".join(usage_msgs)
            keyboard = [[InlineKeyboardButton("⬅️ Back", callback_data="adm_manage_types")]]
            await query.edit_message_text(f"⚠️ Cannot Delete Type\n\nType '{type_name}' is currently used by {usage_str}. Please delete or reassign associated items first.",
                                    reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=None)
        else:
...
async def handle_adm_manage_discounts(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Displays existing GENERAL discount codes and management options."""
    query = update.callback_query
    if query.from_user.id != ADMIN_ID: return await query.answer("Access Denied.", show_alert=True)
    conn = None # Initialize conn
    try:
        ...
        keyboard.extend([
            [InlineKeyboardButton("➕ Add New General Discount Code", callback_data="adm_add_discount_start")],
            [InlineKeyboardButton("⬅️ Back to Admin Menu", callback_data="admin_menu")]
        ])
        try:
            await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=None)
...
async def handle_adm_toggle_discount(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Activates or deactivates a specific GENERAL discount code."""
    query = update.callback_query
    if query.from_user.id != ADMIN_ID: return await query.answer("Access Denied.", show_alert=True)
    ...
    try:
        ...
        # Log action
        action_desc = "GENERAL_DISCOUNT_DEACTIVATE" if new_status == 0 else "GENERAL_DISCOUNT_ACTIVATE"
        log_admin_action(query.from_user.id, action_desc, old_value=current_status, new_value=new_status, reason=f"Code ID: {code_id}")

        action_verb = 'deactivated' if new_status == 0 else 'activated'
        logger.info(f"Admin {query.from_user.id} {action_verb} general discount code ID {code_id}.")
        await query.answer(f"Code {action_verb} successfully.")
...
async def handle_adm_delete_discount(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Handles delete button press for GENERAL discount code, shows confirmation."""
    query = update.callback_query
    if query.from_user.id != ADMIN_ID: return await query.answer("Access Denied.", show_alert=True)
    if not params: return await query.answer("Error: Code ID missing.", show_alert=True)
    conn = None # Initialize conn
    try:
        ...
        msg = (f"⚠️ Confirm Deletion\n\nAre you sure you want to permanently delete general discount code: {code_text}?\n\n"
               f"🚨 This action is irreversible!")
...
async def handle_adm_add_discount_start(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Starts the process of adding a new GENERAL discount code."""
    query = update.callback_query
    if query.from_user.id != ADMIN_ID: return await query.answer("Access Denied.", show_alert=True)
...
async def handle_adm_use_generated_code(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Handles using the suggested random code for GENERAL discounts."""
    query = update.callback_query
    if query.from_user.id != ADMIN_ID: return await query.answer("Access Denied.", show_alert=True)
...
async def handle_adm_set_discount_type(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Sets the GENERAL discount type and asks for the value."""
    query = update.callback_query
    if query.from_user.id != ADMIN_ID: return await query.answer("Access Denied.", show_alert=True)
    ...
    if current_state not in ['awaiting_discount_type', 'awaiting_discount_code']: # Check if state is valid
         ...
         # Try to recover state if code is already set
         ...
         else: # Otherwise go back to main discount menu
    ...
    try:
        ...
        else: await query.answer("Enter the discount value.")
...
async def handle_confirm_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    except telegram_error.BadRequest: await query.answer("Initiating...")

    # Clear context data *before* starting the potentially long async task
    context.user_data.pop('broadcast_target_type', None)
    context.user_data.pop('broadcast_target_value', None)
    context.user_data.pop('broadcast_content', None)
    ...
    try:
        ...
        for i, user_id in enumerate(user_ids):
            try:
                ...
                elif text: await bot.send_message(chat_id=user_id, text=text, parse_mode=None, disable_web_page_preview=True)
                else: logger.warning(f"Broadcast skip for user {user_id}: No text or valid media."); fail_count += 1; continue # Skip if no content
                success_count += 1
            except telegram_error.BadRequest as e:
                 error_str = str(e).lower()
                 if "chat not found" in error_str or "user is deactivated" in error_str or "bot was blocked" in error_str or "peer id invalid" in error_str:
                 ...
                 try: # Retry send after sleep
                     ...
                     else: logger.warning(f"Broadcast retry skip for user {user_id}: No content."); fail_count += 1; continue
                     success_count += 1
                 except Exception as retry_e:
                     logger.error(f"Broadcast fail after retry for {user_id}: {retry_e}"); fail_count += 1;
            ...
            # Minimal sleep between messages to avoid hitting secondary rate limits
...
async def handle_confirm_yes(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    conn = None # Initialize conn
    admin_log_data = {'admin_id': user_id, 'action': action_type.upper()} # Prepare data for logging
    ...
    try:
        ...
        if action_type == "delete_city":
             ...
             admin_log_data['old_value'] = f"City ID: {city_id_str}, Name: {city_name}"
             if city_name:
        ...
        # --- Delete District Logic ---
             ...
             admin_log_data['old_value'] = f"City: {city_name} (ID {city_id_str}), District: {district_name} (ID {dist_id_str})"
        ...
        # --- Delete Product Logic ---
             ...
             admin_log_data['old_value'] = f"Product ID: {product_id}"
        ...
        # --- Delete Product Type Logic ---
              ...
              admin_log_data['old_value'] = f"Product Type: {type_name}"
              # Double-check usage before deleting
              c.execute("SELECT COUNT(*) FROM products WHERE product_type = ?", (type_name,))
              prod_count = c.fetchone()[0]
              c.execute("SELECT COUNT(*) FROM reseller_discounts WHERE product_type = ?", (type_name,))
              reseller_count = c.fetchone()[0]
              if prod_count == 0 and reseller_count == 0:
                  ...
                  conn.rollback()
                  usage_str = " and ".join(filter(None, [f"{prod_count} product(s)" if prod_count else None, f"{reseller_count} reseller rule(s)" if reseller_count else None]))
                  success_msg = f"❌ Error: Cannot delete type '{type_name}' as it is used by {usage_str}."
        ...
        # --- Delete Reseller Discount Rule Logic ---
             ...
             if len(action_params) < 2: raise ValueError("Missing reseller_id/product_type")
             reseller_id = int(action_params[0]); p_type = action_params[1]
             admin_log_data['target_user_id'] = reseller_id
             admin_log_data['old_value'] = f"Reseller Discount Rule - Type: {p_type}"
             delete_res = c.execute("DELETE FROM reseller_discounts WHERE reseller_user_id = ? AND product_type = ?", (reseller_id, p_type))
             if delete_res.rowcount > 0:
                 conn.commit()
                 success_msg = f"✅ Reseller discount rule for '{p_type}' deleted!"
                 next_callback = f"reseller_manage_specific|{reseller_id}"
             else: conn.rollback(); success_msg = f"❌ Error: Reseller discount rule not found."
        # --- END Delete Reseller Discount Rule ---
        ...
        # Log the action if transaction was successful
        if conn.in_transaction: pass # Should not happen if commit/rollback handled correctly
        elif 'success_msg' in locals() and '✅' in success_msg:
             log_admin_action(**admin_log_data)
        else: # Action failed before commit/rollback or success msg indicates failure
            logger.warning(f"Admin action '{action_type}' failed, not logging to admin_log.")
...
async def handle_adm_add_city_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        context.user_data.pop("state", None)
        # Log action
        log_admin_action(user_id, "CITY_ADD", new_value=f"{text} (ID: {new_city_id})")
...
async def handle_adm_add_district_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        new_district_id = c.lastrowid
        conn.commit()
        load_all_data() # Reload global data
        context.user_data.pop("state", None); context.user_data.pop("admin_add_district_city_id", None)
        # Log action
        log_admin_action(user_id, "DISTRICT_ADD", reason=f"City: {city_name} (ID: {city_id_str})", new_value=f"{text} (ID: {new_district_id})")
...
async def handle_adm_edit_district_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        context.user_data.pop("state", None); context.user_data.pop("edit_city_id", None); context.user_data.pop("edit_district_id", None)
        # Log action
        log_admin_action(user_id, "DISTRICT_EDIT", reason=f"City: {city_name} (ID: {city_id_str}), Dist ID: {dist_id_str}", old_value=old_district_name, new_value=new_name)
...
async def handle_adm_edit_city_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        context.user_data.pop("state", None); context.user_data.pop("edit_city_id", None)
        # Log action
        log_admin_action(user_id, "CITY_EDIT", reason=f"City ID: {city_id_str}", old_value=old_name, new_value=new_name)
...
async def handle_adm_bot_media_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Log action
        log_admin_action(user_id, "BOT_MEDIA_SET", new_value=f"{new_media_type} - {final_media_path}")
        ...
        if 'temp_download_path' in locals() and await asyncio.to_thread(os.path.exists, temp_download_path):
             try: await asyncio.to_thread(os.remove, temp_download_path)
             except OSError as e: logger.warning(f"Could not remove temp dl file '{temp_download_path}' in finally: {e}")
...
async def handle_adm_add_type_emoji_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Basic emoji validation (length 1 is a simple proxy)
    if len(emoji) != 1: # TODO: Add more robust emoji validation if needed
    ...
    try:
        ...
        # Log action
        log_admin_action(user_id, "PRODUCT_TYPE_ADD", new_value=f"{type_name} ({emoji})")
...
async def handle_adm_edit_type_emoji_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = None; old_emoji = PRODUCT_TYPES.get(type_name, DEFAULT_PRODUCT_EMOJI) # Get old emoji for logging
    try:
        conn = get_db_connection()
        c = conn.cursor()
        ...
        if update_result.rowcount == 0:
            ...
            load_all_data()
            # Log action
            log_admin_action(user_id, "PRODUCT_TYPE_EDIT_EMOJI", reason=f"Type: {type_name}", old_value=old_emoji, new_value=new_emoji)
...
# --- Message Handlers for General Discount Creation ---
async def process_discount_code_input(update: Update, context: ContextTypes.DEFAULT_TYPE, code_text: str):
    """Shared logic to process entered/generated GENERAL discount code and ask for type."""
    chat_id = update.effective_chat.id
    query = update.callback_query
    user_id = update.effective_user.id
    ...
    try:
        ...
        if c.fetchone():
            error_msg = f"❌ Error: General Discount code '{code_text}' already exists."
            if query:
            ...
            # Keep state awaiting_discount_code if failed uniqueness check from message handler
            if not query: context.user_data['state'] = 'awaiting_discount_code'
            return
    except sqlite3.Error as e:
...
async def handle_adm_discount_code_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles the admin entering the GENERAL discount code text via message."""
    user_id = update.effective_user.id
    if user_id != ADMIN_ID: return
...
async def handle_adm_discount_value_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles the admin entering the GENERAL discount value and saves the code."""
    user_id = update.effective_user.id
    chat_id = update.effective_chat.id
    if user_id != ADMIN_ID: return
    ...
    try:
        ...
        new_code_id = c.lastrowid
        conn.commit()

        # Log action
        log_admin_action(user_id, "GENERAL_DISCOUNT_ADD", reason=f"Type: {dtype}", new_value=f"{code} ({value})", amount_change=value)

        logger.info(f"Admin {user_id} added general discount code: {code} ({dtype}, {value})")
        context.user_data.pop("state", None); context.user_data.pop("new_discount_info", None)
        await send_message_with_retry(context.bot, chat_id, f"✅ General Discount code '{code}' added!", parse_mode=None)
        keyboard = [[InlineKeyboardButton("🏷️ View General Discounts", callback_data="adm_manage_discounts")]]
        await send_message_with_retry(context.bot, chat_id, "Returning to general discount management.", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=None)
